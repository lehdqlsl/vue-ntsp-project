<template>
  <el-tabs v-model="select" :stretch="false"
    ...
    <el-tab-pane label="레이더" name="init" lazy>
      <card class="flex-fill">
        <template slot="header">
          <span class="card-title">레이더 모니터링</span>
          <Radar v-if="statLoading" :data="radar"></Radar>
        </template>
      </card>

    </el-tab-pane>

    <el-tab-pane label="활동량"  lazy>
      <card class="flex-fill">
        <template slot="header">
...
<script>
  ...
  import Radar from 'src/components/user/RadarChart'
